#!/usr/bin/env python3
"""
BLE 广播包深度扫描器 v6.6 (支持目标设备过滤)
新增功能：在控制面板中输入目标设备（MAC地址或名称关键词，逗号分隔），仅捕获匹配的设备广播。
"""
...
# ======================================================================
# 第二部分：Manufacturer ID 映射表（双保险识别）
# ======================================================================
MANUFACTURER_ID_MAP = {
...
# ======================================================================
# 第三部分：本地海量 OUI 数据库解析核心类（V6.4 终极精准修正）
# ======================================================================
...
# ======================================================================
# 新增：双保险厂商识别函数（整合 OUI + Manufacturer ID）
# ======================================================================
def enrich_vendor(adv_info):
    """为 adv_info 添加 vendor 字段"""
    mac = adv_info.get("mac", "")
    vendor_name = "未知厂商"

    # 第一保险：MAC 前缀匹配（公共 MAC）
    if mac and len(mac) >= 8:
        mac_upper = mac.upper()
        if OUI_DB.is_loaded:
            if len(mac_upper) >= 14 and mac_upper[:14] in OUI_DB.vendors:
                vendor_name = OUI_DB.vendors[mac_upper[:14]]
            elif len(mac_upper) >= 11 and mac_upper[:11] in OUI_DB.vendors:
                vendor_name = OUI_DB.vendors[mac_upper[:11]]
            elif len(mac_upper) >= 8 and mac_upper[:8] in OUI_DB.vendors:
                vendor_name = OUI_DB.vendors[mac_upper[:8]]
    
    # 第二保险：Manufacturer ID 匹配（若 OUI 未匹配或未加载）
    if vendor_name == "未知厂商" or vendor_name == "未加载数据库":
        mfg_data = adv_info.get("manufacturer_data", {})
        if mfg_data:
            mfg_id = next(iter(mfg_data.keys()))
            if mfg_id in MANUFACTURER_ID_MAP:
                vendor_name = MANUFACTURER_ID_MAP[mfg_id]

    adv_info["vendor"] = vendor_name
...
class BLEScannerGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("BLE 深度扫描仪 v6.6 (支持目标设备过滤)")
        ...
        self.target_devices = tk.StringVar(value="")  # 新增：目标设备输入
    ...
    def create_widgets(self):
        ...
        # ---- 新增：目标设备输入框 ----
        ttk.Label(row2, text="目标设备 (逗号分隔):").pack(side="left", padx=(10,2))
        self.target_entry = ttk.Entry(row2, textvariable=self.target_devices, width=25)
        self.target_entry.pack(side="left", padx=2)
        ttk.Label(row2, text="(留空=全部)").pack(side="left")
        # -------------------------
    ...
    def reparse_mac_vendors(self):
        ...
        for item_id in self.tree.get_children():
            ...
            # 重新生成 vendor_info
            vendor_info = self._build_vendor_info(adv_info)
            self.tree.set(item_id, column="MAC类型/厂商", value=vendor_info)
            count += 1
        
        self.status_label.config(text=f"✅ 重解析完成，已更新 {count} 条 MAC 地址厂商信息！")

    def _build_vendor_info(self, adv_info):
        """根据 adv_info 生成 'MAC类型/厂商' 字符串"""
        mac = adv_info.get("mac", "")
        mac_type = get_mac_type(mac)
        vendor = adv_info.get("vendor", "未知厂商")
        return f"{mac_type} - {vendor}"
    ...
    def on_tree_select(self, event):
        ...
        if adv_info:
            ...
            self.detail_text.insert(tk.END, f"• MAC 归属厂商: {adv_info.get('vendor', '未知')}\n")
    ...
    def update_gui(self):
        ...
        while count < 300 and not self.raw_queue.empty():
            try:
                ...
                # 此时 adv_info 已有 vendor 字段（由 callback 添加）
    ...
    def start_scan(self):
        ...
        if target:
            self.status_label.config(text=f"扫描中... 目标设备: {target}")
        else:
            self.status_label.config(text="扫描中... (全部设备)")
    ...
    async def _async_scan(self, loop, rssi_min, rssi_max, scan_mode):
        def callback(device, advertisement_data):
            try:
                ...
                # ---------- 新增：目标设备过滤 ----------
                target_str = self.target_devices.get().strip()
                if target_str:
                    # 解析逗号分隔的目标列表
